...
export class AnalyticsService {
  ...
  async report(user:AuthUser,filters:AnalyticsFilters){
    ...
    const ticketTypeName=filters.module==='SERVICE_REQUEST'?'SERVICE_REQUEST':filters.module==='PROBLEM'?'PROBLEM':filters.module==='CHANGE'?'CHANGE':'INCIDENT';
    ...
    const [tickets,groups,priorities,assignees]=await Promise.all([
      this.prisma.ticket.findMany({where,select:{id:true,ticketNumber:true,title:true,createdAt:true,status:{select:{name:true}},priority:{select:{id:true,name:true}},assignmentGroup:{select:{id:true,name:true}},assignedTo:{select:{id:true,name:true}},incident:{select:{resolvedAt:true}},problem:{select:{resolvedAt:true}},change:{select:{actualEnd:true}},slas:{select:{status:true,startedAt:true,firstRespondedAt:true,resolvedAt:true,totalPausedSeconds:true}}},orderBy:{createdAt:'desc'}}),
    ...
    const trend=new Map<string,{date:string;created:number;resolved:number}>();for(let cursor=new Date(from);cursor<=to;cursor=new Date(cursor.getTime()+86400000)){const date=cursor.toISOString().slice(0,10);trend.set(date,{date,created:0,resolved:0})}tickets.forEach(t=>{const created=trend.get(t.createdAt!.toISOString().slice(0,10));if(created)created.created++;const resolvedAt=t.incident?.resolvedAt||t.problem?.resolvedAt||t.change?.actualEnd;const date=resolvedAt?.toISOString().slice(0,10);if(date&&trend.has(date))trend.get(date)!.resolved++});
  ...
  async csv(user:AuthUser,filters:AnalyticsFilters){const report=await this.report(user,filters);const quote=(value:unknown)=>`"${String(value??'').replaceAll('"','""')}"`;return ['Module,Number,Title,Status,Priority,Assignment Group,Assignee,Created,Resolved',...report.tickets.map(t=>[report.module,t.ticketNumber,t.title,t.status?.name,t.priority?.name,t.assignmentGroup?.name,t.assignedTo?.name,t.createdAt!.toISOString(),(t.incident?.resolvedAt||t.problem?.resolvedAt||t.change?.actualEnd)?.toISOString()].map(quote).join(','))].join('\n');}
}
